...
for i in range(1):
    for j in range(len(northlimit)):
        ...
        polygonAreaUrl = "http://SDMDataAccess.sc.egov.usda.gov/Spatial/SDMWGS84Geographic.wfs?SERVICE=WFS" \
                         + "&VERSION=1.1.0&REQUEST=GetFeature" \
                         + "&TYPENAME=MapunitPoly" \
                         + "&FILTER=<Filter><BBOX><PropertyName>Geometry</PropertyName><Box srsName='EPSG:4326'>" \
                         + "<coordinates>%s,%s,%s,%s</coordinates>" \
                           % (westlimit[j], southlimit[j], eastlimit[j], northlimit[j]) \
                         + "</Box></BBOX></Filter>&SRSNAME=EPSG:4326&OUTPUTFORMAT=GML2"
        
        soilDataRequestUrl = "http://SDMDataAccess.sc.egov.usda.gov/Spatial/SDMWGS84Geographic.wfs?SERVICE=WFS" \
                             + "&VERSION=1.1.0&REQUEST=GetFeature&TYPENAME=mapunitpolyextended" \
                             + "&FILTER=<Filter><BBOX><PropertyName>Geometry</PropertyName><Box srsName='EPSG:4326'>" \
                             + "<coordinates>%s,%s,%s,%s</coordinates>" \
                               % (westlimit[j], southlimit[j], eastlimit[j], northlimit[j]) \
                             + "</Box></BBOX></Filter>&SRSNAME=EPSG:4326&OUTPUTFORMAT=GML2"
...
for k in range(len(poly_sm)):
    print '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s' \
        % (poly_sm[k], soils_sm[k], poly_md[k], soils_md[k], poly_lg[k], soils_lg[k],
            poly_xl[k], soils_xl[k], poly_xxl[k], soils_xxl[k])
